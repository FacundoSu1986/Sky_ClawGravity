...
## Roo Code Integration (Antigravity 2026)

This project utilizes a bidirectional integration between Antigravity and Roo Code. If you are operating within Roo Code:

1. **Custom Modes**: Ensure you are using the correct Custom Mode defined in `.roomodes` (e.g., Chief Orchestrator, Purple Auditor) for your specific task.
2. **MCP Servers**: Native MCP servers are configured in `.vscode/cline_mcp_settings.json`. Leverage `chrome-devtools-mcp`, `filesystem`, `github`, and `memory` for systemic tasks.
3. **External Agents**: For massive scale tasks, refer to `.agents/skills/external-github-agents/SKILL.md` to instantiate frameworks like **OpenClaw**, **Dify**, **LangGraph**, **MetaGPT**, **Agno**, or legacy ones like AutoGPT and CrewAI.
4. **Rule Sync**: Follow the directives in `.clinerules` as they directly map to Antigravity's `AUDITORIA_TECNICA_INTEGRAL_2026.md` governance.
