...
class TestSetupWizardConfigPath:
    ...
    async def test_setup_saves_to_config_path(
        self, tmp_path: pathlib.Path, aiohttp_client, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """POST /api/setup saves to the default local config path."""
        ...
        def _save(cfg: LocalConfig) -> None:
            save(cfg, config_path)

        monkeypatch.setattr("sky_claw.antigravity.web.app.load_local_config", _load)
        monkeypatch.setattr("sky_claw.antigravity.web.app.save_local_config", _save)
    ...
    async def test_setup_loads_from_config_path(
        ...
        """GET /api/setup reads from the default local config path."""
        from sky_claw.antigravity.web.app import WebApp
