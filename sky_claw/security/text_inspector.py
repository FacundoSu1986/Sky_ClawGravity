...
INJECTION_PATTERNS = [
    (r"(?i)ignore\s+(all\s+)?(previous|prior)\s+instructions", "Bypass de instrucciones previas", "CRITICAL"),
    (r"(?i)system\s*prompt:", "Intento de sobrescritura de prompt de sistema", "HIGH"),
    (r"(?i)\[(INST|INST\b|/INST)\]", "Patrón de instrucción de modelo (Llama/Mistral style)", "HIGH"),
    (r"(?i)!system", "Directiva de sistema inyectada", "CRITICAL"),
    (r"(?i)forgotten\s*your\s*previous\s*commands", "Técnica de amnesia forzada de prompt", "MEDIUM"),
    (r"(?i)as\s*a\s*developer\s*you\s*must", "Ingeniería social de rol (Jailbreak style)", "MEDIUM"),
    (r"(?i)print\s+(the\s+)?(entire|full)?\s*(prompt|initial\s*instruction)", "Intento de extracción de prompt", "HIGH"),
]

# Patrones de Homoglifos y caracteres invisibles sospechosos (Steganography)
SUSPICIOUS_UNICODE = [
    (r"[\u200B-\u200D\u2060\uFEFF]", "Caracteres invisibles detectados (Posible esteganografía)", "MEDIUM"),
    (r"[\u0430\u0435\u043E\u0440\u0441\u0443\u0445]", "Homoglifos cirílicos detectados en texto latino", "LOW"),
]
