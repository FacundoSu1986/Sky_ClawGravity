...
class ModdingToolsAgent:
    ...
    async def translate_path_wsl_to_win(wsl_path: str, timeout: float = 10.0) -> str:
        ...
        try:
            ...
            raise WSLInteropError(
                f"wslpath tardó más de {timeout}s para: {wsl_path}"
            )
    ...
    async def run_loot(self, params: LootExecutionParams) -> dict:
        ...
        try:
            ...
            return {"status": "error", "logs": f"Ejecutable LOOT no encontrado: {loot_exe_wsl}"}
        ...
        try:
            ...
            return {"status": "error", "logs": "LOOT excedió el timeout de 120 segundos"}
