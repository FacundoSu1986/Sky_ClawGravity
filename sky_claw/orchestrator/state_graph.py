...
class StateGraphValidator:
    ...
    _VALID_TRANSITIONS: Dict[SupervisorState, set[SupervisorState]] = {
        ...
        SupervisorState.IDLE: {SupervisorState.WATCHING, SupervisorState.ANALYZING, SupervisorState.DISPATCHING, SupervisorState.ERROR},
        SupervisorState.WATCHING: {SupervisorState.ANALYZING, SupervisorState.IDLE, SupervisorState.ERROR},
        SupervisorState.ANALYZING: {SupervisorState.DISPATCHING, SupervisorState.HITL_WAIT, SupervisorState.COMPLETED, SupervisorState.IDLE, SupervisorState.ERROR, SupervisorState.PATCHING},
        SupervisorState.DISPATCHING: {SupervisorState.COMPLETED, SupervisorState.IDLE, SupervisorState.ERROR, SupervisorState.GENERATING_LODS},
        SupervisorState.HITL_WAIT: {SupervisorState.DISPATCHING, SupervisorState.COMPLETED, SupervisorState.ERROR},
        SupervisorState.COMPLETED: {SupervisorState.IDLE},
        # FASE 1.5: Transiciones de rollback - ERROR puede transicionar a ROLLING_BACK
        SupervisorState.ERROR: {SupervisorState.IDLE, SupervisorState.INIT, SupervisorState.ROLLING_BACK},
        # ROLLING_BACK puede terminar en IDLE (éxito) o ERROR_FATAL (fallo)
        SupervisorState.ROLLING_BACK: {SupervisorState.IDLE, SupervisorState.ERROR_FATAL},
        ...
        SupervisorState.PATCHING: {SupervisorState.COMPLETED, SupervisorState.ERROR, SupervisorState.ROLLING_BACK},
        # FASE 4: Transiciones de generación de LODs
        SupervisorState.GENERATING_LODS: {SupervisorState.COMPLETED, SupervisorState.ERROR, SupervisorState.ROLLING_BACK},
    }
...
if PYDANTIC_AVAILABLE:
    class WorkflowState(BaseModel):
        ...
        def add_transition(self, from_state: SupervisorState, to_state: SupervisorState, reason: str = "") -> None:
...
class StateGraphNodes:
    ...
    def error_fatal_node(state: StateGraphState) -> Dict[str, Any]:
        ...
        return {
            ...
            "rollback_result": {"success": False, "reason": "Rollback failed, fatal error"}
        }
    ...
    def patching_node(state: StateGraphState) -> Dict[str, Any]:
        ...
        target_plugin = event_data.get("target_plugin")
...
class SupervisorStateGraph:
    ...
    def _build_graph(self) -> None:
        ...
        builder.add_conditional_edges(
            SupervisorState.INIT.value,
            lambda s: SupervisorState.IDLE.value
        )
        
        builder.add_conditional_edges(
            SupervisorState.IDLE.value,
            StateGraphEdges.route_from_idle
        )
        
        builder.add_conditional_edges(
            SupervisorState.WATCHING.value,
            StateGraphEdges.route_from_watching
        )
        
        builder.add_conditional_edges(
            SupervisorState.ANALYZING.value,
            StateGraphEdges.route_from_analyzing
        )
        
        builder.add_conditional_edges(
            SupervisorState.DISPATCHING.value,
            StateGraphEdges.route_from_dispatching
        )
        
        builder.add_conditional_edges(
            SupervisorState.HITL_WAIT.value,
            StateGraphEdges.route_from_hitl_wait
        )
        
        builder.add_conditional_edges(
            SupervisorState.ERROR.value,
            StateGraphEdges.route_from_error
        )
        
        # FASE 1.5: Aristas condicionales para rollback
        builder.add_conditional_edges(
            SupervisorState.ROLLING_BACK.value,
            StateGraphEdges.route_from_rolling_back
        )
        
        # FASE 2: Aristas condicionales para parcheo
        builder.add_conditional_edges(
            SupervisorState.PATCHING.value,
            StateGraphEdges.route_from_patching
        )
    ...
    async def _execute_fallback(self, initial_state: Optional[StateGraphState] = None) -> StateGraphState:
        ...
        if state.get("pending_event"):
            ...
            if event in [WorkflowEventType.MODLIST_CHANGED.value, WorkflowEventType.USER_COMMAND.value]:
...
def create_supervisor_state_graph(profile_name: str = "Default") -> SupervisorStateGraph:
    """
